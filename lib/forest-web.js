(function (global, factory) {
  if (typeof define === "function" && define.amd) {
    define(['exports', 'react', 'react-dom', 'superagent', 'lodash', './forest-core', './forest-common'], factory);
  } else if (typeof exports !== "undefined") {
    factory(exports, require('react'), require('react-dom'), require('superagent'), require('lodash'), require('./forest-core'), require('./forest-common'));
  } else {
    var mod = {
      exports: {}
    };
    factory(mod.exports, global.react, global.reactDom, global.superagent, global.lodash, global.forestCore, global.forestCommon);
    global.forestWeb = mod.exports;
  }
})(this, function (exports, _react, _reactDom, _superagent, _lodash, _forestCore, _forestCommon) {
  'use strict';
  ...
  function persist(o) {
    return Promise.resolve(localStorage.setItem(_forestCore2.default.toUID(o.UID), _forestCore2.default.stringify(o))).then(function () {
      return o.UID + ': ' + [].concat(o.is).join(' ');
    });
  }

  function fetch(uid) {
    return Promise.resolve(localStorage.getItem(uid)).then(function (s) {
      return JSON.parse(s);
    });
  }

  function recache() {
    return Promise.resolve(Object.keys(localStorage)).then(function (uids) {
  ...
  try {
    localStorage && _forestCore2.default.setPersistence({ persist: persist, fetch: fetch, query: query, recache: recache });
  } catch (e) {}
  ...
  var Forest = function (_ForestCommon) {
    ...
    _createClass(Forest, [{
      ...
      value: function dropAll(actually) {
        return Promise.resolve(Object.keys(localStorage)).then(function (uids) {
          return console.log(actually ? '*************** dropping' : '(not dropping)', uids) || actually && localStorage.clear();
        });
      }
    }]);
