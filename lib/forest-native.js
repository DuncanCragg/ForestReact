  ...
  var Forest = function (_ForestCommon) {
    ...
    function Forest(props) {
      ...
      _this.handleOpenURL = function (e) {
        return _this.viewing(e.url.replace(/.*?:\/\/.*?\//g, ''));
      };
      ...
      value: function componentDidMount() {
        ...
        if (_reactNative.Platform.OS !== 'ios') {
          _reactNative.Linking.getInitialURL().then(function (url) {
            return _this2.viewing(url.replace(/.*?:\/\/.*?\//g, ''));
          }).catch(function (err) {
            return console.error('unable to get initial URL:', err);
          });
        } else {
          _reactNative.Linking.addEventListener('url', this.handleOpenURL);
        }
      }
    }, {
