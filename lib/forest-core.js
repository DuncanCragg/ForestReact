(function (global, factory) {
  ...
  function persistenceFlush() {
    return Promise.all(Object.keys(toSave).map(function (uid) {
      return getObject(uid).then(function (o) {
        ...
        delete toSave[uid];
        return persistence.persist(o).then(function (r) {
  ...
  function spawnObject(o) {
    ...
    var Notify = o.Notify || [];
    cachePersistAndNotify(Object.assign({ UID: UID, Notify: Notify }, o));
    doEvaluate(UID);
    return UID;
  }
  ...
  function doGet(url) {
    if (!fetching[url]) {
      ...
      network && network.doGet(url).then(function (json) {
        fetching[url] = false;updateObject(url, Object.assign({ Updated: Date.now() }, json));
      }).catch(function (e) {
  ...
  function updateObject(uid, update) {
    ...
    var justupdated = _lodash2.default.isEqual(diff, { Updated: 0 }); // also needed for Version:
