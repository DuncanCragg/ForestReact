(function (global, factory) {
  ...
  var localProps = ['Timer', 'TimerId', 'PK', 'Notifying', 'Alerted', 'Evaluator', 'Cache', 'ReactNotify', 'userState'];
  var notNotifiableProps = ['Timer', 'TimerId', 'PK'];
  var noPersistProps = ['TimerId', 'PK'];
  ...
  function object(u, p, q) {
    var r = function (uid, path, query) {
      ...
      var observesubs = pathbits[0] !== 'Alerted';
      var c = o;
          ...
          if (val.constructor === Array) {
            ...
            var _r = val.filter(function (v) {
              ...
              if (isLink(v) && query.match.constructor === Object) {
                var _p = ensureObjectState(v, observesubs && observingMatcher(query.match) && o);
                if (!_p) return false;
  ...
  function doEvaluate(uid, params) {
    ...
    var Peer = params && params.Peer;
    var Identity = params && params.Identity;
    ...
    for (var i = 0; i < 4; i++) {
      ...
      if (Peer && Identity) {
        o.ObsPeer = Peer;o.ObsIdentity = Identity;
      }
