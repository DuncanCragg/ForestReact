(function (global, factory) {
  ...
  function doGet(url) {
    return _superagent2.default.get(url).timeout({ response: 9000, deadline: 10000 }).set('Notify', _forestCore2.default.notifyUID).then(function (x) {
      return x.body;
    }).catch(function (e) {
      return console.error('doGet', e, url);
    });
  }

  function doPost(o, url) {
    if (!_forestCore2.default.isURL(url)) return;
    var data = _lodash2.default.omit(o, _forestCore2.default.localProps);
    return _superagent2.default.post(url).timeout({ response: 9000, deadline: 10000 }).set('Notify', _forestCore2.default.notifyUID).send(data).then(function (x) {
      return x.body;
    }).catch(function (e) {
      ...
      value: function wsInit(host, port) {
        ...
        ws.onmessage = function (message) {
          ...
          if (json.notifyUID) {
            ...
            console.log('------------ws------------->>\n', message.data, '\n--------------------------->>');
            _forestCore2.default.incomingObject(Object.assign(json, { Remote: json.UID }));
          }
        };
        ...
        ws.onerror = function (e) {
          setTimeout(function () {
            ...
            _this2.wsRetry = Math.min(Math.floor(_this2.wsRetry * 1.5), 10000);
            console.log('retry WebSocket in..', _this2.wsRetry / 1000);
          }, _this2.wsRetry);
        };
      }
    }, {
