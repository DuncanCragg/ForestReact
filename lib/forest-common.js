(function (global, factory) {
  ...
  function doGet(url) {
    return _superagent2.default.get(url).timeout({ response: 9000, deadline: 10000 }).set(clientPeer ? { Authorization: makeHTTPAuth(clientPeer, 'http://host/uid-identity') } : {}).then(function (x) {
      return x.body;
    });
  }
  ...
  function doPost(o, url) {
    ...
    return _superagent2.default.post(url).timeout({ response: 9000, deadline: 10000 }).set(clientPeer ? { Authorization: makeHTTPAuth(clientPeer, 'http://host/uid-identity') } : {}).send(data).then(function (x) {
      return x.body;
    }).catch(function (e) {
